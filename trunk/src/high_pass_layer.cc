...
bool deHighPassLayer::updatePreview(const dePreview* sourcePreview, dePreview* preview)
{
    ...
    dePreview* tmp1 = new dePreview(sourceColorSpace, preview->getSize());
    dePreview* tmp2 = new dePreview(sourceColorSpace, preview->getSize());
    ...
    blend(*sourcePreview, *tmp2, 1.0, *tmp1, c, deBlendDifference, NULL);
...
void deHighPassLayer::enableChannel(int c)
{
    channels.insert(c);
}

void deHighPassLayer::clearEnabledChannels()
{
    channels.clear();
}

const std::set<int>& deHighPassLayer::getEnabledChannels() const
{
