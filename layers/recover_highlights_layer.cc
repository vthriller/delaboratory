...
deRecoverHighlightsLayer::deRecoverHighlightsLayer(deColorSpace _colorSpace, deChannelManager& _channelManager, int _sourceLayer, deLayerStack& _layerStack, deViewManager& _viewManager)
...
deRecoverHighlightsLayer::~deRecoverHighlightsLayer()
{
}

bool deRecoverHighlightsLayer::updateMainImageNotThreadedWay()
{
    deValue r = getNumericValue("radius") * viewManager.getRealScale();

    int channel = getPropertyChoice("channel")->getIndex();

    bool result = shadowsHighlights(r, channel, getSourceImage(), mainLayerImage, false);
